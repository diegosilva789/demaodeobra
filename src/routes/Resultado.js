...
import {
  ...
  ListGroup, ListGroupItem, Card, CardImg, CardTitle,
  CardText, CardBody,
  Label
} from "reactstrap";
...
export default function Home() {
  ...
  const obterAnuncios = async () => {
    var pal = palavras.replace(/\s/g, " & ")
    await api.post("/obterAnunciosPorAnu_titulo", ({ palavras: pal }))
      .then(r => {
        setTotal(r.data)
  ...
  const obterAnunciosPorCat_id = async (request) => {
    var pal = palavras.replace(/\s/g, " & ")
    request = { ...request, palavras: pal }
    await api.post("/obterAnunciosPorAnu_tituloPorCat_id", (request))
      .then(r => {
        setTotal(r.data)
        const slice = r.data.slice((contadorPagina - 1) * 12, contadorPagina * 12);
        setAnuncios(slice)
  ...
  const obterAnunciosPorAnu_uf = async (request) => {
    ...
    var pal = palavras.replace(/\s/g, " & ")
    request = { ...request, palavras: pal }
    await api.post("/obterAnunciosPorAnu_tituloPorAnu_uf", (request))
      .then(r => {
        setTotal(r.data)
        const slice = r.data.slice((contadorPagina - 1) * 12, contadorPagina * 12);
        setAnuncios(slice)
  ...
  const obterAnunciosPorAnu_cidade = async (request) => {
    ...
    var pal = palavras.replace(/\s/g, " & ")
    request = { ...request, palavras: pal }
    await api.post("/obterAnunciosPorAnu_tituloPorAnu_cidade", (request))
      .then(r => {
        setTotal(r.data)
        const slice = r.data.slice((contadorPagina - 1) * 12, contadorPagina * 12);
        setAnuncios(slice)
        ...
        <Container className="p-3 pb-0">
          <Row className="justify-content-center m-0 p-0">
            <Col xs="12" className="h1 p-0 mb-0" >
              Categorias
              <p style={{ fontSize: "0.9rem", fontWeight: 400, paddingTop: "10px" }}>{total.length} anúncios encontrados na busca atual</p>
            </Col>
          </Row>
        </Container>
      </div>
      ...
      <Container className="margem-s">
        <Row>
          <Col sm="3" md="3" lg="3" className="d-none d-md-block pt-5"
            ...
            <Col className="h4 pb-4 ps-3 pe-3">
              Categorias
              <p style={{ fontSize: "0.7rem", paddingTop: "10px" }}>{total.length} anúncios encontrados na busca atual</p>
            </Col>
